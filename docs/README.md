# Documentation

Tài liệu đầy đủ cho dự án Inland Real Estate.

## 📚 Mục lục

### Getting Started
- [Setup Guide](./SETUP.md) - Hướng dẫn setup đầy đủ
- [Quick Start](../QUICK_START.md) - Hướng dẫn nhanh

### Architecture & Design
- [Architecture](./ARCHITECTURE.md) - Kiến trúc hệ thống
- [Project Structure](../PROJECT_STRUCTURE.md) - Cấu trúc dự án

### API Documentation
- [Public API](./API/public-api.md) - API cho public website
- [CMS API](./API/cms-api.md) - API cho CMS dashboard

### Database
- [Schema Documentation](./DATABASE/schema.md) - Database schema
- [Migrations Guide](./DATABASE/migrations.md) - Hướng dẫn migrations

### Development
- [Coding Standards](./DEVELOPMENT/coding-standards.md) - Coding standards
- [Git Workflow](./DEVELOPMENT/git-workflow.md) - Git workflow
- [Testing Guide](./DEVELOPMENT/testing.md) - Testing guide

### Deployment
- [Deployment Guide](./DEPLOYMENT.md) - Hướng dẫn deploy production
- [Troubleshooting](./TROUBLESHOOTING.md) - Xử lý lỗi thường gặp

### Project Documents
- [Project Summary](./PROJECT_SUMMARY.md) - Tóm tắt dự án
- [Project Analysis](./PHAN_TICH_DU_AN.md) - Phân tích dự án
- [Specification](./dac_ta_du_an_web_bds.md) - Đặc tả dự án
- [Checklist](./CHECKLIST.md) - Implementation checklist

